...
根目录 `index.html` 为大交互原型，仅作参考；新功能进 `web/src/features/`。
...
| `web/src/features/footprints/` | 足迹 CRUD（双写 localStorage） |
| `web/src/features/ai-coach/` | 调用 Edge Function |
...
- AI：`supabase/functions/.env` 或 Dashboard / Vercel **Secrets** 中的 `DEEPSEEK_API_KEY` — **禁止** `VITE_` 前缀、禁止进 git
